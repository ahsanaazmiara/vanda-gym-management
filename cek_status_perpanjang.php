<?php
// =========================================================
// BLOK PHP: MENCARI DATA PERPANJANGAN DARI DATABASE (AJAX)
// =========================================================
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'cek_perpanjangan') {
    require 'includes/koneksi.php'; // Pastikan path benar
    header('Content-Type: application/json');

    $email = mysqli_real_escape_string($koneksi, $_POST['email']);

    // Cari data pengajuan terbaru (baik daftar atau perpanjang) dari email tersebut
    $query = "SELECT u.nama_lengkap, u.email, m.* FROM users u 
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $tglBerakhir = $data['tgl_berakhir'] ? date('d M Y', strtotime($data['tgl_berakhir'])) : '-';
        $tglBayar = date('d M Y', strtotime($data['created_at']));
        ...
        echo json_encode([
            ...
            'tglBayar' => $tglBayar,
            'alasan_tolak' => $data['alasan_tolak'] ?? 'Bukti transfer tidak valid.'
        ]);
    } else {
...
<head>
    ...
    <style>
        ...
        :root { --bg-dark: #000000; --primary-red: #8E1616; --accent-gold: #E8C999; --text-light: #F8EEDF; --input-bg: #111111; }
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .status-container { background-color: #0a0a0a; border: 1px solid #333; border-top: 4px solid var(--primary-red); border-radius: 8px; padding: 30px; width: 100%; max-width: 500px; box-shadow: 0 10px 30px rgba(0,0,0,0.8); text-align: center; position: relative; }
        .btn-back-square { width: 44px; height: 44px; background-color: #1a1a1a; border: 1px solid #333; color: var(--accent-gold); border-radius: 4px; display: flex; align-items: center; justify-content: center; text-decoration: none; font-weight: bold; font-size: 1.2rem; margin-bottom: 20px; transition: 0.3s; }
        ...
        .btn-search { width: 100%; background-color: var(--primary-red); color: white; border: none; min-height: 48px; font-size: 1rem; font-weight: bold; border-radius: 4px; cursor: pointer; text-transform: uppercase; transition: 0.3s; }
        .btn-search:hover { background-color: #a81a1a; }
        .result-box { margin-top: 30px; padding: 20px; border-radius: 4px; background: #111; border: 1px solid #222; display: none; text-align: left; }
        .status-badge { display: inline-block; padding: 5px 12px; border-radius: 20px; font-size: 0.85rem; font-weight: bold; margin-top: 10px; }
        ...
        .detail-info { background-color: #1a1a1a; border: 1px dashed #333; border-radius: 4px; padding: 15px; margin: 15px 0; font-size: 0.85rem; color: #ccc; }
        ...
        .btn-outline-gold { display: inline-flex; align-items: center; justify-content: center; gap: 8px; background: transparent; border: 1px solid var(--accent-gold); color: var(--accent-gold); text-decoration: none; padding: 8px 15px; border-radius: 4px; font-weight: bold; font-size: 0.85rem; transition: 0.3s; margin-top: 10px; cursor: pointer; width: 100%; }
        ...
        @media print { body * { visibility: hidden; } .modal-overlay { position: absolute; left: 0; top: 0; padding: 0; background: transparent; } .receipt-card, .receipt-card * { visibility: visible; } .receipt-card { box-shadow: none; max-width: 100%; padding: 0; } .no-print, .close-modal { display: none !important; } }
    </style>
</head>
<body>
    ...
    <div class="status-container">
        ...
        <button class="btn-search" id="btnTeksCek" onclick="cariStatus()">Cek Status</button>
    ...
    <a href="https://wa.me/6282148556601?text=Halo%20Admin%20Vanda%20Gym,%20saya%20ingin%20cek%20status%20perpanjangan." target="_blank" class="wa-btn" title="Hubungi CS via WhatsApp">
    ...
    <script>
        ...
        // =========================================================
        // AJAX: CEK STATUS PERPANJANGAN KE DATABASE (REAL)
        // =========================================================
        function cariStatus() {
            const email = document.getElementById('cekEmail').value.trim();
            const btn = document.getElementById('btnTeksCek');
            ...
            formData.append('action', 'cek_perpanjangan');
            ...
            .then(data => {
                ...
                if (data.status_code === 'tidak_ditemukan') {
                    document.getElementById('resNama').innerText = email;
                    document.getElementById('resStatus').innerHTML = '<span class="status-badge status-rejected">Tidak Ditemukan</span>';
                    document.getElementById('resPesan').innerText = "Sistem tidak menemukan pengajuan perpanjangan untuk email ini.";
                    return;
                }

                document.getElementById('resNama').innerText = data.email;

                if (data.status === 'aktif') {
                    document.getElementById('resStatus').innerHTML = '<span class="status-badge status-active">Perpanjangan Aktif</span>';
                    ...
                    document.getElementById('receiptData').innerHTML = `
                        ...
                        <p><span>Paket</span> <span>${data.namaPaket}</span></p>
                        <p><span>Mulai Berlaku</span> <span>${data.tglMulai}</span></p>
                        <p><span>Berakhir Pada</span> <span>${data.tglBerakhir}</span></p>
                        <hr style="border:1px dashed #000; margin:10px 0;">
                        <p style="font-weight:bold; font-size:1rem;"><span>TOTAL</span> <span>${data.harga}</span></p>
                    `;

                    document.getElementById('resPesan').innerHTML = `
                        <div class="detail-info">
                            <div class="detail-row"><span>Paket:</span><span>${data.namaPaket}</span></div>
                            <div class="detail-row"><span>Masa Aktif Baru:</span><span style="color: var(--accent-gold);">${data.tglBerakhir}</span></div>
                        </div>
                        <button class="btn-outline-gold" onclick="bukaBukti()">🧾 Download E-Receipt</button>
                    `;
                } else if (data.status === 'ditolak') {
                    document.getElementById('resStatus').innerHTML = '<span class="status-badge status-rejected">Perpanjangan Ditolak</span>';
                    document.getElementById('resPesan').innerHTML = `Verifikasi Gagal: <strong>${data.alasan_tolak}</strong>. Silakan hubungi Admin atau ajukan ulang.`;
                } else {
                    document.getElementById('resStatus').innerHTML = '<span class="status-badge status-pending">Menunggu Verifikasi</span>';
                    document.getElementById('resPesan').innerText = "Admin sedang memverifikasi bukti transfer Anda. Silakan cek berkala.";
                }
            })
            .catch(() => {
                btn.innerText = originalText; btn.disabled = false;
