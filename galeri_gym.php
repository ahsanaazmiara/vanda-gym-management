...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .kosong-db { text-align: center; padding: 40px; color: #666; font-style: italic; background: #111; border: 1px dashed #333; border-radius: 8px; }

        /* MODAL LIGHTBOX CSS */
        .lightbox { display: none; position: fixed; z-index: 999; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.9); justify-content: center; align-items: center; flex-direction: column; }
        .lightbox-content { max-width: 90%; max-height: 80vh; border-radius: 8px; border: 2px solid var(--accent-gold); box-shadow: 0 0 30px rgba(0,0,0,0.8); }
        ...
        .lightbox-title { color: var(--accent-gold); margin-top: 15px; font-size: 1.2rem; font-weight: bold; text-transform: uppercase; }
    </style>
</head>
<body>
    ...
    <div class="galeri-container">
        <div class="nav-top">
            ...
            <span style="color: #444; font-size: 0.8rem; font-weight: bold;">Galeri Publik Vanda Gym</span>
        </div>
                ...
                <?php if(!empty($media['alat'])): foreach($media['alat'] as $m): ?>
                    <div class="gallery-item" data-kategori="alat" data-judul="<?= strtolower($m['judul']) ?>" onclick="bukaMedia('<?= $m['file_path'] ?>', '<?= $m['tipe_media'] ?>', '<?= htmlspecialchars($m['judul']) ?>')">
                        ...
                        <div class="item-info">
                            ...
                            <span class="item-category">Fasilitas / Alat</span>
                        </div>
                    </div>
                <?php endforeach; else: ?>
                    <div class="kosong-db" style="grid-column: 1 / -1;">Belum ada media yang diupload.</div>
                <?php endif; ?>
                <div id="emptyAlat" class="empty-state">Media tidak ditemukan.</div>
            </div>
        </div>
            ...
            <div class="gallery-grid" id="gridUpper">
                <?php if(!empty($media['upper'])): foreach($media['upper'] as $m): ?>
                    <div class="gallery-item" data-kategori="upper" data-judul="<?= strtolower($m['judul']) ?>" onclick="bukaMedia('<?= $m['file_path'] ?>', '<?= $m['tipe_media'] ?>', '<?= htmlspecialchars($m['judul']) ?>')">
                        ...
                        <div class="item-info">
                            <span class="item-title"><?= htmlspecialchars($m['judul']) ?></span>
                            <span class="item-category">Upper Body</span>
                        </div>
                    </div>
                <?php endforeach; else: ?>
                    <div class="kosong-db" style="grid-column: 1 / -1;">Belum ada media yang diupload.</div>
                <?php endif; ?>
                <div id="emptyUpper" class="empty-state">Media tidak ditemukan.</div>
            </div>
        </div>
            ...
            <div class="gallery-grid" id="gridLower">
                <?php if(!empty($media['lower'])): foreach($media['lower'] as $m): ?>
                    <div class="gallery-item" data-kategori="lower" data-judul="<?= strtolower($m['judul']) ?>" onclick="bukaMedia('<?= $m['file_path'] ?>', '<?= $m['tipe_media'] ?>', '<?= htmlspecialchars($m['judul']) ?>')">
                        ...
                        <div class="item-info">
                            <span class="item-title"><?= htmlspecialchars($m['judul']) ?></span>
                            <span class="item-category">Lower Body</span>
                        </div>
                    </div>
                <?php endforeach; else: ?>
                    <div class="kosong-db" style="grid-column: 1 / -1;">Belum ada media yang diupload.</div>
                <?php endif; ?>
                <div id="emptyLower" class="empty-state">Media tidak ditemukan.</div>
            </div>
        </div>

    </div>

    <div id="mediaLightbox" class="lightbox" onclick="tutupMedia()">
    ...
    <script>
        ...
        function filterMedia() {
            ...
            document.getElementById('emptyAlat').style.display = (hitungAlat === 0 && itemAlat.length > 0) ? 'block' : 'none';
            document.getElementById('emptyUpper').style.display = (hitungUpper === 0 && itemUpper.length > 0) ? 'block' : 'none';
            document.getElementById('emptyLower').style.display = (hitungLower === 0 && itemLower.length > 0) ? 'block' : 'none';
        ...
        // Logika Menutup Pemutar Video
        function tutupMedia() {
            document.getElementById('mediaLightbox').style.display = 'none';
            document.getElementById('lightboxContainer').innerHTML = ''; // Hapus elemen agar video berhenti play
        }
    </script>
</body>
</html>